import { IconButton, Typography } from "@material-tailwind/react";

export default function ContactUsHero() {
  return (
    <>
      <div className="relative flex flex-col items-center justify-center">
        <div className="flex flex-col items-center justify-center">
          <div className="absolute inset-0 bg-[url('/assets/Black_Desk1_NoBG.png')] bg-cover bg-top bg-no-repeat blur-md" />
          <div className="relative mb-80 mt-64 md:mb-64 md:mt-72">
            <div className="flex items-center px-10">
              <div className="flex flex-col items-center justify-center">
                {/* @ts-ignore */}
                <Typography
                  className="text-4xl font-bold text-white text-shadow-xl md:text-6xl"
                  variant="h1"
                >
                  Contact
                  <span className="text-green-500"> Us</span>
                </Typography>
                {/* @ts-ignore */}
                <Typography
                  variant="lead"
                  className="mt-3 !text-balance text-gray-400 text-shadow-xl md:!text-pretty"
                >
                  Use the methods below to reach out to us.
                </Typography>
              </div>
            </div>
          </div>
          <div className="relative z-10 w-9/12 rounded-xl bg-gray-100 shadow-xl backdrop-blur-3xl backdrop-saturate-200 transition-shadow duration-300 md:container md:w-auto md:shadow-lg md:hover:shadow-2xl">
            <div className="grid grid-cols-1 gap-3 text-center md:grid-cols-4 lg:gap-24">
              <div className="flex flex-col items-center justify-center gap-2 p-6">
                {/* @ts-ignore */}
                <IconButton className="cursor-auto hover:shadow-lg">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="h-7 w-7"
                  >
                    <path
                      fillRule="evenodd"
                      d="m11.54 22.351.07.04.028.016a.76.76 0 0 0 .723 0l.028-.015.071-.041a16.975 16.975 0 0 0 1.144-.742 19.58 19.58 0 0 0 2.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827a8.25 8.25 0 0 0-16.5 0c0 3.846 2.02 6.837 3.963 8.827a19.58 19.58 0 0 0 2.682 2.282 16.975 16.975 0 0 0 1.145.742ZM12 13.5a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"
                      clipRule="evenodd"
                    />
                  </svg>
                </IconButton>
                {/* @ts-ignore */}
                <Typography variant="h5" className="font-semibold text-black">
                  Address
                </Typography>
                {/* @ts-ignore */}
                <Typography
                  variant="paragraph"
                  className="font-medium text-gray-700"
                >
                  vesla 12, khiab ek
                </Typography>
              </div>

              <div className="flex flex-col items-center justify-center gap-2 p-6">
                {/* @ts-ignore */}
                <IconButton className="cursor-auto hover:shadow-lg">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="h-7 w-7"
                  >
                    <path d="M1.5 8.67v8.58a3 3 0 0 0 3 3h15a3 3 0 0 0 3-3V8.67l-8.928 5.493a3 3 0 0 1-3.144 0L1.5 8.67Z" />
                    <path d="M22.5 6.908V6.75a3 3 0 0 0-3-3h-15a3 3 0 0 0-3 3v.158l9.714 5.978a1.5 1.5 0 0 0 1.572 0L22.5 6.908Z" />
                  </svg>
                </IconButton>
                {/* @ts-ignore */}
                <Typography variant="h5" className="font-semibold text-black">
                  Email
                </Typography>
                {/* @ts-ignore */}
                <Typography
                  variant="paragraph"
                  className="font-medium text-gray-700"
                >
                  Example@gmail.com
                </Typography>
              </div>

              <div className="flex flex-col items-center justify-center gap-2 p-6">
                {/* @ts-ignore */}
                <IconButton className="cursor-auto hover:shadow-lg">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="h-7 w-7"
                  >
                    <path
                      fillRule="evenodd"
                      d="M1.5 4.5a3 3 0 0 1 3-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 0 1-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 0 0 6.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 0 1 1.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 0 1-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5Z"
                      clipRule="evenodd"
                    />
                  </svg>
                </IconButton>
                {/* @ts-ignore */}
                <Typography variant="h5" className="font-semibold text-black">
                  Phone Number
                </Typography>
                {/* @ts-ignore */}
                <Typography
                  variant="paragraph"
                  className="font-medium text-gray-700"
                >
                  061 65789312
                </Typography>
              </div>

              <div className="flex flex-col items-center justify-center gap-2 p-6">
                {/* @ts-ignore */}
                <IconButton className="cursor-auto hover:shadow-lg">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="h-7 w-7"
                  >
                    <path
                      fillRule="evenodd"
                      d="M7.5 6a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM3.751 20.105a8.25 8.25 0 0 1 16.498 0 .75.75 0 0 1-.437.695A18.683 18.683 0 0 1 12 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 0 1-.437-.695Z"
                      clipRule="evenodd"
                    />
                  </svg>
                </IconButton>
                {/* @ts-ignore */}
                <Typography variant="h5" className="font-semibold text-black">
                  Contact
                </Typography>
                {/* @ts-ignore */}
                <Typography
                  variant="paragraph"
                  className="font-medium text-gray-700"
                >
                  Ramtin Panahi Bazaz
                </Typography>
              </div>
            </div>
          </div>
        </div>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 1440 200"
          className="-mt-[380px] lg:-mt-[240px]"
        >
          <path
            fill="#f5f5f5"
            fillOpacity="1"
            d="M0,160L60,170.7C120,181,240,203,360,186.7C480,171,600,117,720,96C840,75,960,85,1080,101.3C1200,117,1320,139,1380,149.3L1440,160L1440,320L1380,320C1320,320,1200,320,1080,320C960,320,840,320,720,320C600,320,480,320,360,320C240,320,120,320,60,320L0,320Z"
          ></path>
        </svg>
      </div>
    </>
  );
}

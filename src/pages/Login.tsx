...
export default function Login() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch("/Data/AdminUsers.json");
        if (!response.ok) {
          throw new Error("Failed to fetch Users data from AdminUsers.json");
        }
  ...
  const handleLogin = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (users) {
      ...
      if (matchedUser) {
        ...
        // Optionally, alert or set an error state if credentials are incorrect
